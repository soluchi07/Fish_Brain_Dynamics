...
def parse_args() -> argparse.Namespace:
    ...
    # [UPGRADE 4: Cluster & Multiprocessing]
    p.add_argument(
        "--n-workers",
        type=int,
        default=None,
...
# [UPGRADE 3: Execution Update] Import explicitly for motion correction
...
def load_plane_multi_tp(files: list[str], z_index: int) -> np.ndarray:
    """Load specific z-plane across all multi-tp files."""
    with h5py.File(files[0], "r") as fh:
        Z, H, W = fh["Data"].shape
    if z_index >= Z:
        raise ValueError(f"z_index {z_index} >= Z={Z}")
