"""
orig_pipeline.py  —  Phase 4: Universal CNMF Pipeline with Quality Filters
...
def discover(
    folder: Path, override: Optional[str] = None
...
def bayesian_tune(
    mmap_path: str, dims: tuple[int, int], mask: np.ndarray, tag: str = "tune"
    ...
    def objective(params):
        ...
        print(
            f"raw={n_pre:3d} kept={n_post:3d} composite={metrics['composite_score']:+.4f} t={rt:.0f}s"
        )
    ...
    if not df_counts.empty:
        ...
        ax.bar(
            df_counts_plot["trial"],
            df_counts_plot["input"],
            color="lightgray",
            label="input",
        )
        ax.bar(
            df_counts_plot["trial"],
            df_counts_plot.get("final", 0),
            color="seagreen",
            label="kept",
        )
        ax.set_xlabel("Trial")
...
def mode_time_split():
    ...
    tune_keep, _ = (
        quality_filter(cnmf_tune, dims, mask, int(best_params["gSig"]))
        if cnmf_tune
        else ([], {})
    )
...
def mode_plane_split():
    ...
    cnmf_tune, _ = run_cnmf(best_params, tune_mmap)
    tune_keep, _ = (
        quality_filter(cnmf_tune, dims, tune_mask, int(best_params["gSig"]))
        if cnmf_tune
        else ([], {})
    )
    tune_A = cnmf_tune.estimates.A[:, tune_keep] if cnmf_tune and tune_keep else None
